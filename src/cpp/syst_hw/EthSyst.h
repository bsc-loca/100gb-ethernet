...
class EthSyst {
  ...
  uint32_t* extMem = reinterpret_cast<uint32_t*>(0x70000000);          // a segment of external SDRAM connected directly
  uint32_t* sysMem = reinterpret_cast<uint32_t*>(XPAR_HBM_0_BASEADDR); // same segment of external SDRAM but connected via cache
