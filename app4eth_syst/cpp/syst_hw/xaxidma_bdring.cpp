...
void XAXIDMA_CACHE_FLUSH(XAxiDma_Bd* BdPtr) {
  ethSyst.cacheFlush(size_t(BdPtr));
}
void XAXIDMA_CACHE_FLUSH(UINTPTR BdPtr) {
  ethSyst.cacheFlush(BdPtr);
}
void XAXIDMA_CACHE_INVALIDATE(XAxiDma_Bd* BdPtr) {
  ethSyst.cacheInvalid(size_t(BdPtr));
}
void XAXIDMA_CACHE_INVALIDATE(u32 BdPtr) {
  ethSyst.cacheInvalid(size_t(BdPtr));
}
#endif
...
int XAxiDma_StartBdRingHw(XAxiDma_BdRing *RingPtr)
{
    ...
    usleep(0); // a tiny delay to let BD ring to start, needed for Lagarto-Ox core
