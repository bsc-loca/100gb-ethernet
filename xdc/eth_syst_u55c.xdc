...
set sys_clk [get_clocks -of_objects [get_pins -hierarchical eth100gb/s_axi_aclk  ]]
...
add_cells_to_pblock [get_pblocks eth_subsys]      [get_cells        -hierarchical -filter {NAME =~ *eth_cmac_syst/eth100gb}]
add_cells_to_pblock [get_pblocks eth_subsys]      [get_cells        -hierarchical -filter {NAME =~ *eth_cmac_syst/rx_fifo }]
add_cells_to_pblock [get_pblocks eth_subsys]      [get_cells        -hierarchical -filter {NAME =~ *eth_cmac_syst/tx_fifo }]
add_cells_to_pblock -quiet [get_pblocks eth_slr1] [get_cells        -hierarchical -filter {NAME =~ *eth_cmac_syst/eth_dma }]
add_cells_to_pblock -quiet [get_pblocks eth_slr1] [get_cells -quiet -hierarchical -filter {NAME =~ *eth_cmac_syst/dma_*connect*}]
